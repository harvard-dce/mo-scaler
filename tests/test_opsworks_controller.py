...
class TestOpsworksController(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_boto3 = patch(
            "boto3.client", autospec=True, return_value=mock_opsworks
        )
    ...
    def test_no_admin(self):
        ...
        self.assertRaises(
            OpsworksControllerException, self.controller.__getattribute__, "admin"
        )
    ...
    def test_scale_to(self):
        ...
        self.assertRaises(OpsworksControllerException, self.controller.scale_to, 4)
    ...
    def test_scale_down(self):
        ...
        with patch.object(self.controller, "_get_workers_to_stop") as get_workers:
            ...
            self.assertEqual(
                [1, 1, 0, 0], [x.stop.call_count for x in self.controller._instances]
            )
    ...
    def test_workers_to_stop_uptime_check(self):
        ...
        instances[0]._mock_wraps.ec2_inst = MagicMock(
            launch_time=datetime(2015, 11, 13, 10, 45, 0)
        )
        instances[1]._mock_wraps.ec2_inst = MagicMock(
            launch_time=datetime(2015, 11, 13, 10, 20, 0)
        )
        instances[2]._mock_wraps.ec2_inst = MagicMock(
            launch_time=datetime(2015, 11, 13, 10, 4, 0)
        )
        ...
        with patch.dict(os.environ, {"MOSCALER_IDLE_UPTIME_THRESHOLD": "30"}):
            with freeze_time("2015-11-13 12:00:00"):
                ...
                self.assertEqual(
                    ["3", "2"], [x._mock_wraps.InstanceId for x in to_stop]
                )
    ...
    def test_sort_by_uptime(self):
        ...
        self.assertEqual(
            ["3", "2", "1"],
            [
                x._mock_wraps.InstanceId
                for x in self.controller._sort_by_uptime(instances)
            ],
        )
    ...
    def test_scale_up(self):
        ...
        self.assertEqual(
            [0, 0, 1, 0, 1], [x.start.call_count for x in self.controller._instances]
        )
    ...
    def test_scale_up_prefer_bigger_instances(self):
        ...
        self.assertEqual(
            [0, 1, 0, 0, 0, 0], [x.start.call_count for x in self.controller._instances]
        )
        ...
        self.assertEqual(
            [0, 1, 1, 0, 0, 1], [x.start.call_count for x in self.controller._instances]
        )
